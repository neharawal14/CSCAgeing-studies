...
void pressure_analysis_per_layer(){
...
//
///  TGraph *gr_pressure_slope_plus[6];
  //TGraph *gr_pressure_slope_minus[6];
	TH1D *hist_slope_pressure_endcap_plus[32];
	TH1D *hist_slope_pressure_endcap_plus_outer[32];
	TH1D *hist_slope_pressure_endcap_minus[32];
	TH1D *hist_slope_pressure_endcap_minus_outer[32];
	
	TH1D *hist_chi2_pressure_endcap_plus[32];
	TH1D *hist_chi2_pressure_endcap_minus[32];
  ...
  TString input_file_path = "/cmsuf/data/store/user/t2/users/neha.rawal/CSCAgeing_2022/analysis_2022/2022_output_files_new_way/";
	...
	TString result_path = "/cmsuf/data/store/user/t2/users/neha.rawal/CSCAgeing_2022/analysis_2022/analysis_gas_gain/all_channels_per_layer_plots/";
  
//	TFile *file = TFile::Open(input_file_path+"DUMMYFILENAME_2017_all_"+chamber[i]+"_tree.root";
  TFile *output_file = new TFile(result_path+"output_file_pressure.root","RECREATE");
  ...
  if(debug) std::cout<<"going to start the loop"<<std::endl;
	...
	for(int i=0; i<32;i++){
   	TH1D *hist_slope_pressure_all_layer = new TH1D("hist_slope_pressure_all_layer", "slope distribution : all channels " , 64,-0.016,0); ;
   	TH1D *hist_chi2_pressure_all_layer = new TH1D("hist_chi2_pressure_all_layer", "chi2/ndf distribution : all channels " , 30,0,15); ;
		Float_t _slope, _slope_error, _chi2;
		Int_t _ndof, _chamber,_endcap;
	  Int_t	_layer;

		TGraphErrors *gr_pressure_slope_plus[6];
 		TGraphErrors *gr_pressure_slope_minus[6];
 		TGraphErrors *gr_pressure_chi2_plus[6];
 		TGraphErrors *gr_pressure_chi2_minus[6];

    THStack* hs_plus = new THStack("hs_plus", "Slope distribution");
    THStack* hs_minus = new THStack("hs_minus", "Slope distribution");
    TH1D *hist_pressure_slope_per_layer_endcap[6];
    TH1D *hist_pressure_slope_per_layer_endcap_plus[6];
    TH1D *hist_pressure_slope_per_layer_endcap_minus[6];

    TH1D *hist_pressure_chi2_per_layer_endcap[6];
    TH1D *hist_pressure_chi2_per_layer_endcap_plus[6];
    TH1D *hist_pressure_chi2_per_layer_endcap_minus[6];

		TString hist_string = TString::Format("hist_slope_pressure_endcap_plus[%d]",i);
	  hist_slope_pressure_endcap_plus[i] = new TH1D(hist_string, " slope of gas gain dependence ", 64,-0.016,0);
		hist_string = TString::Format("hist_slope_pressure_endcap_plus_outer[%d]",i);
	  hist_slope_pressure_endcap_plus_outer[i] = new TH1D(hist_string, " slope of gas gain dependence ", 64,-0.016,0);
		hist_string = TString::Format("hist_slope_pressure_endcap_minus[%d]",i);
	  hist_slope_pressure_endcap_minus[i] = new TH1D(hist_string, " slope of gas gain dependence ", 64,-0.016,0);
		hist_string = TString::Format("hist_slope_pressure_endcap_minus_outer[%d]",i);
	  hist_slope_pressure_endcap_minus_outer[i] = new TH1D(hist_string, " slope of gas gain dependence ", 64,-0.016,0);

 	  hist_string = TString::Format("hist_chi2_pressure_endcap_plus[%d]",i);
	  hist_chi2_pressure_endcap_plus[i] = new TH1D(hist_string, " chi2 of gas gain dependence ", 30,0,15);
		hist_string = TString::Format("hist_chi2_pressure_endcap_minus[%d]",i);
	  hist_chi2_pressure_endcap_minus[i] = new TH1D(hist_string, " chi2 of gas gain dependence ",30,0,15);


		for (int points = 0; points < 6; points++) {
			  gr_pressure_slope_plus[points] = new TGraphErrors();
			  gr_pressure_slope_minus[points] = new TGraphErrors();
			  gr_pressure_chi2_plus[points] = new TGraphErrors();
			  gr_pressure_chi2_minus[points] = new TGraphErrors();
		}

		for (int points = 0; points < 6; points++) {
			  hist_string = TString::Format("hist_pressure_slope_per_layer_endcap[%d]",points);
				hist_pressure_slope_per_layer_endcap[points] = new TH1D(hist_string," slope per layer ", 64, -0.016,0);

			  hist_string = TString::Format("hist_pressure_slope_per_layer_endcap_plus[%d]",points);
				hist_pressure_slope_per_layer_endcap_plus[points] = new TH1D(hist_string," slope per layer ", 64, -0.016,0);
  			hist_string = TString::Format("hist_pressure_slope_per_layer_endcap_minus[%d]",points);
				hist_pressure_slope_per_layer_endcap_minus[points] = new TH1D(hist_string," slope per layer ", 64, -0.016,0);

				hist_string = TString::Format("hist_pressure_chi2_per_layer_endcap[%d]",points);
				hist_pressure_chi2_per_layer_endcap[points] = new TH1D(hist_string," chi2 per layer ",30,0,15);

			  hist_string = TString::Format("hist_pressure_chi2_per_layer_endcap_plus[%d]",points);
				hist_pressure_chi2_per_layer_endcap_plus[points] = new TH1D(hist_string," chi2 per layer ",30,0,15);
  			hist_string = TString::Format("hist_pressure_chi2_per_layer_endcap_minus[%d]",points);
				hist_pressure_chi2_per_layer_endcap_minus[points] = new TH1D(hist_string," chi2 per layer ", 30,0,15);
...
//  input_file_chamber[i] = input_file_path + "DUMMYFILENAME_2017_all_"+chamber[i]+"_tree.root";outf_dataset25to36fbonly_ME11a_output.root
	...
	//std::cout<<"input file "<<file<<std::endl; 
	 tree_pressure = (TTree*) file->Get("tree__pressure");	
	 tree_pressure->SetBranchAddress("_slope",&_slope);
	 tree_pressure->SetBranchAddress("_slope_error",&_slope_error);
	 tree_pressure->SetBranchAddress("_layer",&_layer);
	 tree_pressure->SetBranchAddress("_chamber",&_chamber);
	 tree_pressure->SetBranchAddress("_endcap",&_endcap);
	 tree_pressure->SetBranchAddress("_chi2",&_chi2);
	 tree_pressure->SetBranchAddress("_ndof",&_ndof);


		//ofstream text_file; 
		//text_file.open("ME12HV1_test.txt");
	 std::cout<<"entering into tree"<<std::endl; 
	 for(int entry=0; entry<tree_pressure->GetEntries(); entry++) {
    tree_pressure->GetEntry(entry);
	   ...
	   if(_layer ==k+1 && _endcap==1) {
          hist_slope_pressure_all_layer->Fill(_slope); 

         hist_chi2_pressure_all_layer->Fill(chi2_ndof); 
		     //text_file<<" chamber "<<_chamber<<" endcap "<<_endcap<<" layer "<<_layer<<" slope "<<_slope<<std::endl; 	 
				 hist_pressure_slope_per_layer_endcap_plus[k]->Fill(_slope);
				 hist_pressure_slope_per_layer_endcap[k]->Fill(_slope);
    		 gr_pressure_slope_plus[k]->SetPoint(_chamber,_chamber,_slope);
	 	  	 gr_pressure_slope_plus[k]->SetPointError(_chamber,0, _slope_error);

				 hist_pressure_chi2_per_layer_endcap_plus[k]->Fill(chi2_ndof);
				 hist_pressure_chi2_per_layer_endcap[k]->Fill(chi2_ndof);
    		 gr_pressure_chi2_plus[k]->SetPoint(_chamber,_chamber,chi2_ndof);
	 			 gr_pressure_chi2_plus[k]->SetPointError(_chamber,0, 0);
...
//		     std::cout<<" chamber "<<_chamber<<" endcap "<<_endcap<<" layer "<<_layer<<" slope "<<_slope<<std::endl; 	 
         ...
         hist_chi2_pressure_all_layer->Fill(chi2_ndof); 
		     hist_pressure_slope_per_layer_endcap_minus[k]->Fill(_slope);
				 hist_pressure_slope_per_layer_endcap[k]->Fill(_slope);
	    	 std::cout<<" channe; "<<_chamber<<" endcap "<<_endcap<<" layer "<<_layer<<" slope "<<_slope<<std::endl; 	 
		     gr_pressure_slope_minus[k]->SetPoint(_chamber,_chamber,_slope);
	 			 gr_pressure_slope_minus[k]->SetPointError(_chamber,0, _slope_error);

				 hist_pressure_chi2_per_layer_endcap_minus[k]->Fill(chi2_ndof);
				 hist_pressure_chi2_per_layer_endcap[k]->Fill(chi2_ndof);
    		 gr_pressure_chi2_minus[k]->SetPoint(_chamber,_chamber,chi2_ndof);
	 			 gr_pressure_chi2_minus[k]->SetPointError(_chamber,0, 0);

					}
	 }
	 if(_layer ==1 ||_layer==6) {
		 if(_endcap==1)  hist_slope_pressure_endcap_plus_outer[i]->Fill(_slope);
		 else hist_slope_pressure_endcap_minus_outer[i]->Fill(_slope);
	 }
	 if(_layer ==2 ||_layer==3 || _layer==4 || _layer==5 ) {
		    if(_endcap==1)  hist_slope_pressure_endcap_plus[i]->Fill(_slope);
		    if(_endcap==2) 	 hist_slope_pressure_endcap_minus[i]->Fill(_slope);
	 }
	 ...
	 for(int k=0; k<6; k++){	
		 ...
		 plotting_graph(gr_pressure_slope_plus[k],layer_nb_plus,chamber[i] );
		 plotting_graph(gr_pressure_slope_minus[k], layer_nb_minus,chamber[i]);
	   plotting_histogram(hist_pressure_slope_per_layer_endcap_plus[k], layer_nb_plus,chamber[i]);
	   plotting_histogram(hist_pressure_slope_per_layer_endcap_minus[k], layer_nb_minus,chamber[i]);

		 TString layer_nb = TString::Format("layer_%d_both",k+1);
	   plotting_histogram(hist_pressure_slope_per_layer_endcap[k], layer_nb,chamber[i]);

		 plotting_graph_chi2(gr_pressure_chi2_plus[k],layer_nb_plus,chamber[i] );
		 plotting_graph_chi2(gr_pressure_chi2_minus[k], layer_nb_minus,chamber[i]);
	   plotting_histogram_chi2(hist_pressure_chi2_per_layer_endcap_plus[k], layer_nb_plus,chamber[i]);
	   plotting_histogram_chi2(hist_pressure_chi2_per_layer_endcap_minus[k], layer_nb_minus,chamber[i]);
	   plotting_histogram_chi2(hist_pressure_chi2_per_layer_endcap[k], layer_nb,chamber[i]);


	 }


	   plotting_histogram(hist_slope_pressure_all_layer, "all_chamber_slope",chamber[i]);
	   plotting_histogram_chi2(hist_chi2_pressure_all_layer, "all_chamber_chi2",chamber[i]);

	 hist_slope_pressure_endcap_plus[i]->SetFillColor(kBlue);
	 hist_slope_pressure_endcap_plus_outer[i]->SetFillColor(kRed);
	 hist_slope_pressure_endcap_minus[i]->SetFillColor(kBlue);
	 hist_slope_pressure_endcap_minus_outer[i]->SetFillColor(kRed);

//	 hist_slope_pressure_endcap_plus[i]->SetStats(1);
//	 hist_slope_pressure_endcap_plus_outer[i]->SetStats(1);
//	 hist_slope_pressure_endcap_minus[i]->SetStats(1);
//	 hist_slope_pressure_endcap_minus_outer[i]->SetStats(1);

/*	 hist_slope_pressure_endcap_plus[i]->SetStatFormat("Variable 1: Mean=%f, RMS=%f");
	 hist_slope_pressure_endcap_plus_outer[i]->SetStatFormat("Variable 2: Mean=%f, RMS=%f");
	 hist_slope_pressure_endcap_minus[i]->SetStatFormat("Variable 3: Mean=%f, RMS=%f");
	 hist_slope_pressure_endcap_minus_outer[i]->SetStatFormat("Variable 4: Mean=%f, RMS=%f");

	 hist_slope_pressure_endcap_plus[i]->SetStatTextColor(kBlue); 
	 hist_slope_pressure_endcap_plus_outer[i]->SetStatTextColor(kRed); 
	 hist_slope_pressure_endcap_minus[i]->SetStatTextColor(kBlue); 
	 hist_slope_pressure_endcap_minus_outer[i]->SetStatTextColor(kRed); 
	 ...
	 hist_slope_pressure_endcap_plus[i]->SetLabel(label1);
	 hist_slope_pressure_endcap_plus_outer[i]->SetLabel(label2);
	 hist_slope_pressure_endcap_minus[i]->SetLabel(label3);
	 hist_slope_pressure_endcap_minus_outer[i]->SetLabel(label4);
*/

   c_first = new TCanvas("c_first"," canvas firsrt" );
	 c_first->cd(); 
	 hist_slope_pressure_endcap_plus[i]->Draw();
   c_first->Update();
	 TPaveStats *st1 = (TPaveStats*)hist_slope_pressure_endcap_plus[i]->FindObject("stats");
	 c_first->Close();

	 c_second = new TCanvas("c_second"," canvas firsrt" );
	 c_second->cd(); 
	 hist_slope_pressure_endcap_plus_outer[i]->Draw();
	 c_second->Update();
   TPaveStats *st2 = (TPaveStats*)hist_slope_pressure_endcap_plus_outer[i]->FindObject("stats");
	 hist_slope_pressure_endcap_plus_outer[i]->Draw();
	 c_second->Close();

	 c_first_minus = new TCanvas("c_first_minus"," canvas firsrt" );
	 c_first_minus->cd(); 
	 hist_slope_pressure_endcap_minus[i]->Draw();
   c_first_minus->Update();
	 TPaveStats *st3 = (TPaveStats*)hist_slope_pressure_endcap_minus[i]->FindObject("stats");
	 c_first_minus->Close();

	 c_second_minus = new TCanvas("c_second_minus"," canvas fsecond" );
	 c_second_minus->cd(); 
	 hist_slope_pressure_endcap_minus_outer[i]->Draw();
   c_second_minus->Update();
	 TPaveStats *st4 = (TPaveStats*)hist_slope_pressure_endcap_minus_outer[i]->FindObject("stats");
	 c_second_minus->Close();

	 
   hs_plus->Add(hist_slope_pressure_endcap_plus_outer[i]);
   hs_plus->Add(hist_slope_pressure_endcap_plus[i]);

	 hs_minus->Add(hist_slope_pressure_endcap_minus_outer[i]);
   hs_minus->Add(hist_slope_pressure_endcap_minus[i]);
...
//	 gStyle->SetOptStat();
	 hs_plus->Draw();
	 hs_plus->GetXaxis()->SetTitle("Slope of charge  vs Pressure (/hPa)");
...
//TPaveStats *st1 = (TPaveStats*)hist_slope_pressure_endcap_plus[i]->FindObject("stats");
//TPaveStats *st2 = (TPaveStats*)hist_slope_pressure_endcap_plus_outer[i]->FindObject("stats");
//TPaveStats *st2 = (TPaveStats*)hist_slope_pressure_endcap_plus_outer[i]->GetListOfFunctions()->FindObject("stats");

//TPaveStats *st2 = (TPaveStats*)hs_plus->GetStack()->FindObject("hist_slope_pressure_endcap_plus_outer[i]")->FindObject("stats"); 
//c->Modified();
	 c->SaveAs("./all_channels_per_layer_plots/histogram_"+chamber[i]+"_plus.pdf");
	 c->Close();
	 ...
	 c1->cd();
	 hs_minus->Draw();
	 hs_minus->GetXaxis()->SetTitle("Slope of charge  vs Pressure (/hPa)");
	 c1->Update();
	 st3->Draw(); 
	 st4->Draw(); 
	 c1->SaveAs("./all_channels_per_layer_plots/histogram_"+chamber[i]+"_minus.pdf");
	 c1->Close();
	 ...
	 //if(_layer ==2 && endcap==2) hist_slope_pressure_layer2_endcap_plus[i]->Fill(_slope);
	 //if(_layer ==3 && endcap==2) hist_slope_pressure_layer3_endcap_plus[i]->Fill(_slope);	 std::cout<<"existing filling entrues"<<std::endl;
	 //if(_layer ==4 && endcap==2) hist_slope_pressure_layer4_endcap_plus[i]->Fill(_slope);	 }
	 //if(_layer ==5 && endcap==2) hist_slope_pressure_layer5_endcap_plus[i]->Fill(_slope);//	hist_slope_pressure_2[i] = (TH1D*) file->Get("h_slope_pressure_2__dataset25to36fbonly");	
	 //if(_layer ==6 && endcap==2) hist_slope_pressure_layer6_endcap_plus[i]->Fill(_slope);//  hist_slope_instlumi[i] = (TH1D*) file->Get("h_slope_instlumi__dataset25to36fbonly");	
...
//	gStyle->SetOptFit(1011);
//	hist_slope_pressure_layer1[i]->Fit(gaus1_layer1,"R");
//  gStyle->SetOptFit(1011);                
//	hist_slope_pressure_layer2[i]->Fit(gaus1_layer2,"R");
//  gStyle->SetOptFit(1011);                
//	hist_slope_pressure_layer3[i]->Fit(gaus1_layer3,"R");
//  gStyle->SetOptFit(1011);               
//	hist_slope_pressure_layer4[i]->Fit(gaus1_layer4,"R");
//  gStyle->SetOptFit(1011);              
//	hist_slope_pressure_layer5[i]->Fit(gaus1_layer5,"R");
//  gStyle->SetOptFit(1011);             
//	hist_slope_pressure_layer6[i]->Fit(gaus1_layer6,"R");

/*	mean_slope_pressure[i] = gaus1->GetParameter(1);
	mean_slope_pressure_unc[i] = gaus1->GetParameter(2);
	
	hist_slope_pressure_2[i]->Fit(gaus1_2,"R");
 	
	mean_slope_pressure_2[i] = gaus1_2->GetParameter(1);
	mean_slope_pressure_unc_2[i] = gaus1_2->GetParameter(2);
...
/*
  ...
  c_pressure->cd();
	hist_slope_pressure[i]->GetXaxis()->SetRangeUser(-0.016, 0.0);
...
void plotting_histogram(TH1D *h, TString layer_nb, TString chamber_name){	
	...
	TF1 *f = new TF1("f","gaus",-0.016,0);
  h->Fit(f,"R");
	gStyle->SetOptFit(1011);  
  TCanvas *c_pressure = new TCanvas("c_pressure","pressure plot of i th file after pressure correction for first time",1300,1000);
  c_pressure->cd();
...
//  gr->GetXaxis()->SetBinLabel(gr->GetXaxis()->FindBin(i + 1.), chamber[i]); // Find out which bin on the x-axis the point corresponds to and set the bin label
	h->GetXaxis()->SetTitle("Slope of charge vs pressure (/hPa)");
	h->GetYaxis()->SetTitle("Nb of channels");
  h->Draw();
  c_pressure->SaveAs("./all_channels_per_layer_plots/pressure_"+chamber_name+"_per_"+layer_nb+"layer.pdf");
  c_pressure->Close();
}
...
//  gr->GetXaxis()->SetBinLabel(gr->GetXaxis()->FindBin(i + 1.), chamber[i]); // Find out which bin on the x-axis the point corresponds to and set the bin label
	...
	h->GetYaxis()->SetTitle("Nb of channels");
  h->Draw();
  c_pressure->SaveAs("./all_channels_per_layer_plots/chi2_"+chamber_name+"_per_"+layer_nb+"layer.pdf");
  c_pressure->Close();
}
void plotting_graph(TGraphErrors *gr, TString layer_nb, TString chamber_name){	
  ...
  c_pressure->cd();
	...
	//gr->GetXaxis()->SetTickLength(1);
	TAxis *axis_pressure_1 = gr->GetXaxis();
	axis_pressure_1->SetLabelSize(0.03);
//  gr->GetXaxis()->SetBinLabel(gr->GetXaxis()->FindBin(i + 1.), chamber[i]); // Find out which bin on the x-axis the point corresponds to and set the bin label
	gr->GetYaxis()->SetTitle("slope of Charge vs pressure (/hPa)");
	gr->GetYaxis()->SetTitleOffset(1.5);
	gr->GetXaxis()->SetTitle("chamber number");
  gr->Draw("AP*");
  c_pressure->SaveAs("./all_channels_per_layer_plots/slope_pressure_"+chamber_name+"_"+layer_nb+".pdf");
  c_pressure->Close();
}
void plotting_graph_chi2(TGraphErrors *gr, TString layer_nb, TString chamber_name){	
  TCanvas *c_pressure = new TCanvas("c_pressure","chi 2 /ndf ",1300,1000);
  c_pressure->cd();
...
//  gr->GetXaxis()->SetBinLabel(gr->GetXaxis()->FindBin(i + 1.), chamber[i]); // Find out which bin on the x-axis the point corresponds to and set the bin label
	...
	gr->GetXaxis()->SetTitle("chamber number");
  gr->Draw("AP*");
  c_pressure->SaveAs("./all_channels_per_layer_plots/chi2_pressure_"+chamber_name+"_"+layer_nb+".pdf");
  c_pressure->Close();
}
...
/*
  ...
  c_pressure_2->cd();
	hist_slope_pressure_2[i]->GetXaxis()->SetRangeUser(-0.001, 0.001);
