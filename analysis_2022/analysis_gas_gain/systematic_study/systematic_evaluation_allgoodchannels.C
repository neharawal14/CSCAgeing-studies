...
void systematic_evaluation_allgoodchannels(TString chamber, int channels_total, float channel_nb){
...
*/
 // TGraphErrors *graph_slope_integratelumi_chamber_initial;
	...
	TString result_path = "/cmsuf/data/store/user/t2/users/neha.rawal/CSCAgeing_2022/analysis_2022/analysis_gas_gain/systematic_study/result_output/"; 
  ...
  TDirectory * dir_intlumi;
 	...
 	float new_hist_value_error = 0;
      ...
      std::cout<<"issue after opening file"<<std::endl;
    	...
    	std::cout<<"example file name : DUMMYFILENAME_2017_all_ME13HV1_tree14to26fbonly_7e33to9e33trimmean_chamber34_layer4_Endcap2vs_integratelumi"<<std::endl;
 int layer_nb = 6;
int endcap_nb =2;  
  ...
  for(int channel=1; channel<=channel_nb;channel++){
        ...
        for(int layer=1; layer<=layer_nb;layer++){
          TString hist_org_name = TString::Format("_dataset25to36fbonlytrimmean_chamber%d_layer%d_Endcap%dvs_integratelumi",channel,layer,endcap);
...
/*This was for a test purpose about how is ME13HV3 distribution and are values correct 
 * TCanvas *c = new TCanvas("c","normalized plot",700,700);
 c->cd();
 integratelumi_ME13HV3->Draw();
 c->SaveAs("/cmsuf/data/store/user/t2/users/neha.rawal/CSC_2017_data_RAW_RECO/nTuple_output/2017/analyzing_gas_gain/systematic_study/ME13HV3_allgoodchannels.pdf");
	ofstream file_ME13HV3;
  file_ME13HV3.open("ME13HV3_values.txt"); 
*/
  ...
  std::cout<<"channel totla "<<channels_total<<std::endl;
	for(int i =0; i<channels_total; i++){
//	for(int i =0; i<3; i++){
			if(integratelumi.at(i)==NULL) { std::cout<<"print channel .........................................................................................................        number not there "<<i<<std::endl; 
				continue;}
   	...
   	for(int j=1; j<= integratelumi.at(i)->GetNbinsX()+1 ; j++){
	    ...
	    if (integratelumi.at(i)->GetBinContent(j) == 0 || integratelumi_ME13HV3->GetBinContent(j) ==0) continue;
      new_hist_value = integratelumi.at(i)->GetBinContent(j)/integratelumi_ME13HV3->GetBinContent(j);
	    new_hist_value_error =  new_hist_value * sqrt(pow(integratelumi.at(i)->GetBinError(j)/integratelumi.at(i)->GetBinContent(j),2) +pow(integratelumi_ME13HV3->GetBinError(j)/integratelumi_ME13HV3->GetBinContent(j),2) ) ;
 // new_hist_value = j;
