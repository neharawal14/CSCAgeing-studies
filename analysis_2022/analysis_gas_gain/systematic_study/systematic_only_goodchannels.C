...
#include <iostream>
void systematic_only_goodchannels(TString chamber, int channels_total){
...
*/
 // TGraphErrors *graph_slope_integratelumi_chamber_initial;
	...
	TString result_path = "/cmsuf/data/store/user/t2/users/neha.rawal/CSCAgeing_2022/analysis_2022/analysis_gas_gain/systematic_study/result_systematic/"; 
  ...
  TDirectory * dir_intlumi;
 	...
 	float new_hist_value_error = 0;
      ...
      std::cout<<"issue after opening file"<<std::endl;
    	...
    	std::cout<<"example file name : DUMMYFILENAME_2017_all_ME13HV1_tree15to23fbonly_7e33to10e33trimmean_chamber34_layer4_Endcap2vs_integratelumi"<<std::endl;
...
*/
  ...
  std::cout<<"channel total "<<channels_total<<std::endl;
	for(int i =0; i<channels_total; i++){
//	for(int i =0; i<3; i++){
...
output_file->cd();
new_integratelumi.at(0).SetName("intlumi_only_goodchannels");
   new_integratelumi.at(0).Write();
	 output_file->Close();
 // file->Close();
