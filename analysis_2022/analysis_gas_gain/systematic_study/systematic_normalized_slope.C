void systematic_normalized_slope(){
...
// TString chamber[32] = {"ME11a", "ME11b", "ME12HV1", "ME12HV2","ME12HV3", "ME13HV1","ME13HV2","ME13HV3","ME21HV1","ME21HV2", "ME21HV3", "ME22HV1", "ME22HV2", "ME22HV3", "ME22HV4","ME22HV5","ME31HV1","ME31HV2", "ME31HV3", "ME32HV1", "ME32HV2", "ME32HV3", "ME32HV4","ME32HV5","ME41HV1","ME41HV2", "ME41HV3", "ME42HV1", "ME42HV2", "ME42HV3", "ME42HV4","ME42HV5" };  
	bool debug = false;
     ...
     TString chamber[32] = {"ME11a", "ME11b", "ME12HV1", "ME12HV2","ME12HV3", "ME13HV1","ME13HV2","ME13HV3","ME21HV1","ME21HV2", "ME21HV3", "ME22HV1", "ME22HV2", "ME22HV3", "ME22HV4","ME22HV5","ME31HV1","ME31HV2", "ME31HV3", "ME32HV1", "ME32HV2", "ME32HV3", "ME32HV4","ME32HV5","ME41HV1","ME41HV2", "ME41HV3", "ME42HV1", "ME42HV2", "ME42HV3", "ME42HV4", "ME42HV5"};
	TString input_file_path = "/cmsuf/data/store/user/t2/users/neha.rawal/CSCAgeing_2022/analysis_2022/analysis_gas_gain/systematic_study/result_output/";
	...
	TString result_path = "/cmsuf/data/store/user/t2/users/neha.rawal/CSCAgeing_2022/analysis_2022/analysis_gas_gain/systematic_study/results_normalized/";
...
/*
  ...
  if(debug) std::cout<<"going to start the loop"<<std::endl;
	...
	for(int i=0; i<32;i++){
		TString string_name = TString::Format("hist_slope[%d]",i);
	 hist_slope[i]	= new TH1F(string_name, "Slope wrt int lumi ", 200,-0.005,0.005);
 }	
	TF1 *gaus_function = new TF1("gaus_function","gaus",-0.002,0.002);
...
//	for(int i=0; i<2;i++){
  if(debug) std::cout<<"will open root file "<<std::endl;
	std::cout<<input_file_path<<"/output_file.root"<<std::endl;
  input_file = TFile::Open(input_file_path+"output_file_"+chamber[i]+".root");
  if(debug) std::cout<<"after opening root file"<<input_file<<std::endl;

	// to put slopes separately for each layer
	 tree = (TTree*) input_file->Get("tree_intlumi");
   
		TBranch *b_chi2;
		TBranch *b_ndf;
		TBranch *b_slope;
		TBranch *b_slope_error;
    ...
    Float_t chi2;
     ...
     b_chi2 = tree->GetBranch("chi2");    
	   b_chi2->SetAddress(&chi2);
     b_ndf = tree->GetBranch("ndf");    
	   b_ndf->SetAddress(&ndf);
     b_slope = tree->GetBranch("slope");    
	   b_slope->SetAddress(&slope);
     b_slope_error = tree->GetBranch("slope_error");    
	   b_slope_error->SetAddress(&slope_error);

  
  	 std::cout<<" number of entries in tree pressure "<<tree->GetEntries()<<std::endl;
...
//      std::cout<<" chi square value  "<<chi2<<" ndf "<<ndf<<std::endl;
//      std::cout<<" slope  "<<slope<<std::endl;
			hist_slope[i]->Fill(slope);
     }
	  	gaus_function->SetParNames("Constant", "Mean","Sigma");
	  	c->SetName(chamber[i]);
  c->cd();
  gStyle->SetOptFit(1011);
	hist_slope[i]->SetTitle(chamber[i]);
	hist_slope[i]->GetXaxis()->SetTitle("Slope of Q vs #int L (/fb)");
	hist_slope[i]->Fit(gaus_function,"R");
  hist_slope[i]->Draw();
  c->SaveAs(result_path+"slope_"+chamber[i]+".pdf");
  c->Clear();
	mean_slope[i]  = gaus_function->GetParameter(1);
	mean_slope_error[i]  = gaus_function->GetParError(1);
	sigma_slope[i]  = gaus_function->GetParameter(2);
