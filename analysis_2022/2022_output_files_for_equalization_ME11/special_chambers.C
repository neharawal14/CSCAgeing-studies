#define special_chambers_cxx
#include "special_chambers.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
void special_chambers :: Loop(){
   // we are passing name of the input file when calling the macro 
   TFile * inputf = TFile::Open("/cmsuf/data/store/user/t2/users/neha.rawal/CSCAgeing_2022/analysis_2022/2022_output_files_for_equalization_ME11/output_after_pressure_depenedence_removal_ME11.root");
...
//   TFile * outf  = new TFile(output_path+"outf"+detregionstr+"_"+chamber_string_name+"_output.root","recreate");  
     ...
     hcharge->Fill(_rhsumQ_RAW,rhidreduced);
   }
  ...
  params_mean = trimmed_mean(hcharge); 
	...
	TH1D *h_gas_gain_plus = new TH1D("h_gas_gain_plus", " Gas gain : +endcap : chamber ME11",200,0,1000); 
	TH1D *h_gas_gain_minus = new TH1D("h_gas_gain_minus", " Gas gain : -endcap : chamber ME11",200,0,1000); 
      ...
      TString rhidshort ="chamber"+ (TString) Form("%d", chambernb)  +"_layer"+ (TString)Form("%d",h%10) + endcap;
       ...
       HV0[h] = 2900 +delta_HV0_value;
         ...
         gas_gain_file<<" channel : "<<h<<" rhid : "<<rhidshort<<" charge : "<<params_mean[h]<<std::endl;
			 ...
			 out_text<<std::setw(22)<<rhidshort<<std::setw(10)<<sign*floor(abs(delta_HV0_value))<<std::setw(10)<<HV0[h]<<std::setw(20)<<chamber_string<<std::setw(10)<<(TString)Form("%d",h%10)<<std::setw(10)<<t<<std::endl;
...
*/
		 ...
		 TH1D *hcharge_special_chamber9_layer2_endcap2_third = new TH1D("hcharge_special_chamber9_layer2_endcap2_third","",200,0,1000);
		 TString intlumi_string;
     ...
     int rhidreduced = ((int)floor(_rhid/10))%1000;
     if(_rhid> 2000000) rhidreduced +=400;
		 int endcap = (rhidreduced<400)? 1:2; 
		 if(rhidreduced>400) rhidreduced -=400; 
     int layer = rhidreduced%10;
	   int chamber = floor(rhidreduced/10); 

		 //std::cout<<" print "<<layer<<" layer"<<endcap<<" endcap"<<chamber<<" chamber"<<std::endl;
		 if(6<= _integratelumi && _integratelumi<7 && chamber==9 && layer==2 &&endcap==2){
		 //if(chamber==9 && layer==2 &&endcap==2){
			// std::cout<<" etry inside dersierd "<<std::endl;
		 hcharge_special_chamber9_layer2_endcap2_first->Fill(_rhsumQ_RAW);
		 intlumi_string = "6_intlumi_7";
		 }
		 if(22<= _integratelumi && _integratelumi<23 && chamber==9 && layer==2 && endcap==2){
		 hcharge_special_chamber9_layer2_endcap2_second->Fill(_rhsumQ_RAW);
		 intlumi_string = "22_intlumi_23";
			 //std::cout<<" etry inside dersierd "<<std::endl;
     }
		 if(2<= _integratelumi && _integratelumi<3 && chamber==9 && layer==2 &&endcap==2){
		 //if(chamber==9 && layer==2 &&endcap==2){
			// std::cout<<" etry inside dersierd "<<std::endl;
		 hcharge_special_chamber9_layer2_endcap2_third->Fill(_rhsumQ_RAW);
		 intlumi_string = "2_intlumi_3";
		 }


   }
		 hcharge_special_chamber9_layer2_endcap2_first->SetTitle("6<= intlumi <7");
		 hcharge_special_chamber9_layer2_endcap2_second->SetTitle("22<= intlumi <23");
		 hcharge_special_chamber9_layer2_endcap2_third->SetTitle("2<= intlumi <3");
      ...
      TCanvas *c2 = new TCanvas("c2", "Charge in chamber before trimming ");
			c2->cd();
			hcharge_special_chamber9_layer2_endcap2_first->Draw();
			c2->Update();
			gStyle->SetOptStat(111111111);
			c2->SaveAs(gas_gain_plots+"special_charge_initial_total_chamber9_first_before_trimming.pdf");

			gStyle->SetOptStat(111111111);
      TCanvas *c1 = new TCanvas("c1", "Charge in chamber before trimming ");
			c1->cd();
			hcharge_special_chamber9_layer2_endcap2_second->Draw();
			c1->Update();
			gStyle->SetOptStat(111111111);
			c1->SaveAs(gas_gain_plots+"special_charge_initial_total_chamber9_second_before_trimming.pdf");
     TCanvas *c = new TCanvas("c", "Charge in chamber before trimming ");
			c->cd();
			hcharge_special_chamber9_layer2_endcap2_third->Draw();
			c->Update();
			gStyle->SetOptStat(111111111);
			c->SaveAs(gas_gain_plots+"special_charge_initial_total_chamber9_third_before_trimming.pdf");

		 float parameter_special_first;
		 float parameter_special_second;
		 float parameter_special_third;
		 ...
		 parameter_special_first = trimmed_mean_special(hcharge_special_chamber9_layer2_endcap2_first,9,2,2,intlumi_string);
		 parameter_special_second = trimmed_mean_special(hcharge_special_chamber9_layer2_endcap2_second,9,2,2,intlumi_string);
		 parameter_special_third = trimmed_mean_special(hcharge_special_chamber9_layer2_endcap2_third,9,2,2,intlumi_string);

		 //std::cout<<"special parameter gas gain chamber9, layer 2 endcap2"<<parameter_special_first<<" : "<<parameter_special_second<<std::endl;
	
}
/*std::vector<float> special_chambers :: trimmed_mean(TH2D *myh){
...
*/
float special_chambers :: trimmed_mean_special(TH1D *myh,int chambernb,int endcap,int layer, TString intlumi){
    
		 float result =0 ; //Assume that fitted function has two parameters
      //Skipping empty entries
      
      TString rhidshort ="chamber"+ (TString) Form("%d", chambernb)  +"_layer"+ (TString)Form("%d",layer) +"_endcap"+ (TString) Form("%d",endcap);
      //Declare histo to store trimmed mean for different values of the variable of interest (pressure, inst L,...)
     ...
     for(int it = 1; it<=   proj_charge->GetNbinsX() ;it++) {
      ...
      TCanvas *c2 = new TCanvas("c2", "Charge in chamber before trimming ");
			...
			c2->Update();
			gStyle->SetOptStat(111111111);
			c2->SaveAs(gas_gain_plots+"special_charge_initial_total_"+rhidshort+"_"+intlumi+"_after_trimming.pdf");
