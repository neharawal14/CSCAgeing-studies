...
#ifndef special_chambers_h
#define special_chambers_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   special_chambers(TTree *tree=0);
   virtual ~special_chambers();
   ...
   virtual void     Show(Long64_t entry = -1);
	 std::vector<float>  trimmed_mean(TH2D*);
	  ...
	  TString output_plots_folder = "/cmsuf/data/store/user/t2/users/neha.rawal/CSCAgeing_2022/analysis_2022/2022_output_files_for_equalization_ME11/special_results_ME11/";   
		TString chamber_string_name = "ME11";
		TString gas_gain_plots  = output_plots_folder;
		TString gas_gain_plots_all  = output_plots_folder+"charge_plots_all/";
		float trimmean = 0.7;
 
		float trimmed_mean_special(TH1D *myh,int chambernb,int endcap,int layer,TString);
};

#endif

#ifdef special_chambers_cxx
special_chambers::special_chambers(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
}

special_chambers::~special_chambers()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t special_chambers::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t special_chambers::LoadTree(Long64_t entry)
{
...
void special_chambers::Init(TTree *tree)
{
...
Bool_t special_chambers::Notify()
{
...
void special_chambers::Show(Long64_t entry)
{
...
Int_t special_chambers::Cut(Long64_t entry)
{
...
#endif // #ifdef special_chambers_cxx
